...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X = pack_padded_sequence(X, source_lengths, enforce_sorted=True)
        enc_output, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_output) # (src_len, b, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, h*2)
        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=1))  # (b, h)
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=1))  # (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        encoder_hiddens_proj = self.att_projection(enc_hiddens)  # (b, src_len, h)
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            dec_state, o_t, _ = self.step(Ybar_t, dec_state, enc_hiddens, encoder_hiddens_proj, enc_masks)
