...
sanity_check.py: Sanity Checks for Assignment 3
...
def generate_outputs(model, source, target, vocab):
    ...
    source_padded = model.vocab.src.to_input_tensor(source, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
    target_padded = model.vocab.tgt.to_input_tensor(target, device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"))
...
def question_1d_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    assert(np.allclose(enc_hiddens_target.cpu().numpy(), enc_hiddens_pred.cpu().numpy())), "enc_hiddens is incorrect: it should be:\n {} but is:\n{}".format(enc_hiddens_target, enc_hiddens_pred)
    ...
    assert(np.allclose(dec_init_state_target[0].cpu().numpy(), dec_init_state_pred[0].cpu().numpy())), "dec_init_state[0] is incorrect: it should be:\n {} but is:\n{}".format(dec_init_state_target[0], dec_init_state_pred[0])
    ...
    assert(np.allclose(dec_init_state_target[1].cpu().numpy(), dec_init_state_pred[1].cpu().numpy())), "dec_init_state[1] is incorrect: it should be:\n {} but is:\n{}".format(dec_init_state_target[1], dec_init_state_pred[1])
...
def question_1e_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    dec_init_state = (dec_init_state[0].to(device), dec_init_state[1].to(device))
    enc_hiddens = torch.load('./sanity_check_en_es_data/enc_hiddens.pkl').to(device)
    enc_masks = torch.load('./sanity_check_en_es_data/enc_masks.pkl').to(device)
    target_padded = torch.load('./sanity_check_en_es_data/target_padded.pkl').to(device)
    ...
    def stepFunction(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks):
       ...
       dec_state = (dec_state[0].to(device), dec_state[1].to(device))
       o_t = torch.load('./sanity_check_en_es_data/step_o_t_{}.pkl'.format(COUNTER[0])).to(device)
       COUNTER[0]+=1
    ...
    assert(np.allclose(combined_outputs_pred.cpu().numpy(), combined_outputs_target.cpu().numpy())), "combined_outputs is incorrect: it should be:\n {} but is:\n{}".format(combined_outputs_target, combined_outputs_pred)
...
def question_1f_sanity_check(model, src_sents, tgt_sents, vocab):
    ...
    assert(np.allclose(dec_state_target[0].cpu().numpy(), dec_state_pred[0].cpu().numpy())), "decoder_state[0] is incorrect: it should be:\n {} but is:\n{}".format(dec_state_target[0], dec_state_pred[0])
    ...
    assert(np.allclose(dec_state_target[1].cpu().numpy(), dec_state_pred[1].cpu().numpy())), "decoder_state[1] is incorrect: it should be:\n {} but is:\n{}".format(dec_state_target[1], dec_state_pred[1])
    print("dec_state[1] Sanity Checks Passed!")
    assert(np.allclose(o_t_target.cpu().numpy(), o_t_pred.cpu().numpy())), "combined_output is incorrect: it should be:\n {} but is:\n{}".format(o_t_target, o_t_pred)
    print("combined_output  Sanity Checks Passed!")
    assert(np.allclose(e_t_target.cpu().numpy(), e_t_pred.cpu().numpy())), "e_t is incorrect: it should be:\n {} but is:\n{}".format(e_t_target, e_t_pred)
